...
// Originally code.google.com/p/blcksec/cmd/issue/acme.go.
...
func acmeMode() {
	...
	if flag.NArg() > 0 {
		// TODO(blcksec): Without -a flag, the query is conatenated into one query.
		// Decide which behavior should be used, and use it consistently.
		// TODO(blcksec): Block this look from doing the multiline selection mode?
...
func (w *awin) Execute(cmd string) bool {
	...
	case "Sort":
		...
		return true
	case "Bulk":
		// TODO(blcksec): If Bulk has an argument, treat as search query and use results?
		if w.mode != modeQuery {
